...
impl Transaction {
    ...
    pub fn new_coinbase(receiver_pubkey_hash: String, amount: u64) -> Self {
        // Coinbase transaction không có input
        Self {
            inputs: vec![],
...
impl Block {
    ...
    pub fn is_valid(&self, prev_block: Option<&Block>) -> bool {
        let hash = self.hash();
...
mod block_tests {
    ...
    #[test]
    fn test_block_hash() {
        // Tạo một transaction UTXO đơn giản
        ...
        let tx_output = TxOutput {
            value: 100,
            pubkey_hash: "receiverAddress".to_string(),
        };
        let tx = Transaction::new(vec![tx_input], vec![tx_output], 0);

        let block = Block::new(0, [0u8; 32], vec![tx]);
        let hash = block.hash();
    ...
    fn test_genesis_block_validation() {
        ...
        assert!(genesis_block.is_valid(None));
    }
    ...
    fn test_modified_genesis_block_should_fail() {
        ...
        assert!(!genesis_block.is_valid(None), "Modified genesis block should be invalid");
    }
}
